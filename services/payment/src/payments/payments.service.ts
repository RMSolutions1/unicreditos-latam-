...
export class PaymentsService {
  ...
  /**
   * Acreditación real de un pago aprobado -- compartida entre proveedores (Mercado Pago y
   * AstroPay hoy) porque el negocio es exactamente el mismo: ledger, cuota, crédito y audit no
   * pueden divergir según quién procesó el pago (master prompt §39).
   */
  private async approveIntent(externalReference: string, providerPaymentId: string, ctx: SessionContext) {
    const intentSummary = await this.prisma.client.paymentIntent.findUnique({ where: { externalReference } })
    if (!intentSummary || intentSummary.status === 'APPROVED') return
    ...
    const approvedNotification = await this.prisma.client.$transaction(async (tx) => {
      ...
      await tx.paymentIntent.update({ where: { id: intent.id }, data: { status: 'APPROVED', providerPaymentId } })
      ...
      await tx.auditLog.create({
        data: {
          ...
          after: { providerPaymentId, amount: intent.amount },
  ...
  private async rejectOrExpireIntent(externalReference: string, status: 'REJECTED' | 'CANCELLED' | 'EXPIRED') {
    await this.prisma.client.paymentIntent.updateMany({
      where: { externalReference, status: { notIn: ['APPROVED'] } },
      data: { status },
    })
  }
  ...
  async handleMercadoPagoWebhook(headers: Record<string, string | undefined>, body: { data?: { id?: string }; id?: string }, queryId: string | undefined, ctx: SessionContext) {
    ...
    if (payment.status === 'approved' && payment.externalReference) {
      await this.approveIntent(payment.externalReference, payment.providerPaymentId, ctx)
    } else if (['rejected', 'cancelled'].includes(payment.status) && payment.externalReference) {
      await this.rejectOrExpireIntent(payment.externalReference, payment.status === 'rejected' ? 'REJECTED' : 'CANCELLED')
    }
  ...
  /**
   * A diferencia de Mercado Pago, el callback de AstroPay ya trae el estado final completo --
   * no hace falta un getPayment de vuelta. Firma RSA verificada contra el body crudo (ver
   * AstroPayAdapter.verifyCallbackSignature) antes de tocar cualquier dato.
   */
  async handleAstroPayWebhook(headers: Record<string, string | undefined>, rawBody: string, ctx: SessionContext) {
    const signatureValid = await this.astroPay.verifyCallbackSignature(headers, rawBody)
    if (!signatureValid) throw new DomainError('WEBHOOK_SIGNATURE_INVALID', 'Firma de webhook inválida.')

    let body: { payment_external_id?: string; merchant_payment_id?: string; payment_status?: string }
    try {
      body = JSON.parse(rawBody)
    } catch {
      throw new DomainError('VALIDATION_ERROR', 'Callback de AstroPay con body inválido.')
    }

    const eventId = String(body.payment_external_id ?? body.merchant_payment_id ?? '')
    if (!eventId || !body.merchant_payment_id) return { ok: true }

    const existing = await this.prisma.client.webhookEvent.findUnique({ where: { provider_providerEventId: { provider: 'astropay', providerEventId: eventId } } })
    if (existing?.processedAt) return { ok: true, idempotent: true }

    const event =
      existing ??
      (await this.prisma.client.webhookEvent.create({ data: { provider: 'astropay', providerEventId: eventId, signatureValid: true, payload: body as object } }))

    if (body.payment_status === 'COMPLETED') {
      await this.approveIntent(body.merchant_payment_id, body.payment_external_id ?? eventId, ctx)
    } else if (body.payment_status === 'REJECTED') {
      await this.rejectOrExpireIntent(body.merchant_payment_id, 'REJECTED')
    } else if (body.payment_status === 'EXPIRED') {
      await this.rejectOrExpireIntent(body.merchant_payment_id, 'EXPIRED')
    }
