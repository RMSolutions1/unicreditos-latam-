...
- [x] `AstroPayAdapter` (`services/payment/src/providers/astropay.adapter.ts`) — construido contra la documentación real de AstroPay (OAuth client_credentials, `POST /v1/payments` Offsite Checkout, verificación de firma RSA vía `GET /v1/certificates`). Auth probada en vivo (200 OK contra `partners-api.astropay.com`). **Wireado en `PaymentRouterService`** detrás de `ENABLE_ASTROPAY` (default `false`): decisión de negocio tomada — AstroPay **convive** con Mercado Pago para `AR` (no lo reemplaza), el cliente elige el proveedor al crear el `PaymentIntent` (`POST /payment-intents { installmentId, provider? }`), sin preferencia el default es Mercado Pago. **Riesgo pendiente sin mitigar**: las credenciales recibidas autentican contra producción, no sandbox — nunca se probó una creación de pago ni un webhook de punta a punta. El algoritmo de firma (RSA-SHA256) y las URLs de `api.astropay.com`/`v1/payments`/`v1/certificates` son inferencias razonables de la documentación, no confirmadas contra una respuesta real. `ENABLE_ASTROPAY` debe permanecer en `false` hasta decidir asumir ese riesgo en producción sin haber podido probar en sandbox.
