...
- [x] `AstroPayAdapter` (`services/payment/src/providers/astropay.adapter.ts`) — construido contra la documentación real de AstroPay (OAuth client_credentials, `POST /v1/payments` Offsite Checkout, verificación de firma RSA vía `GET /v1/certificates`). **Wireado en `PaymentRouterService`** detrás de `ENABLE_ASTROPAY` (default `false`): decisión de negocio tomada — AstroPay **convive** con Mercado Pago para `AR` (no lo reemplaza), el cliente elige el proveedor al crear el `PaymentIntent` (`POST /payment-intents { installmentId, provider? }`), sin preferencia el default es Mercado Pago.
  - **Confirmado en vivo contra sandbox real (2026-09-06)**, con credenciales de prueba provistas por el negocio: un único host, `partners-api.astropay.com`, sirve auth (`POST /v1/partners/oauth/token`, 200 OK), `GET /v1/certificates` (200 OK, certificado real recibido) y `POST /v1/payments` (endpoint existe y responde). La tabla "Environments" de la doc pública (`partners-api-sandbox.astropay.com`, `api.astropay.com`/`api-sandbox.astropay.com`) resultó incorrecta o inaplicable para este set de credenciales — se corrigió el adapter para usar el host único (`ASTROPAY_BASE_URL`, sin split sandbox/producción: cada credencial ya apunta a su ambiente del lado de AstroPay).
  - **Bloqueante nuevo y puntual**: `POST /v1/payments` devuelve `400 { "detail": { "method": "is required" } }` — un campo no documentado en la tabla de Request Parameters de "Creating Payments" (probablemente el "Integration Mode" OFFSITE/EMBEDDED mencionado como precondición en otra página, sin nombre de campo confirmado). No se adivinó el valor — falta una llamada más de prueba contra sandbox para confirmarlo antes de completar un pago de punta a punta.
  - El algoritmo de firma del webhook (RSA-SHA256) sigue siendo una inferencia de "RSA-based signature" sin confirmar contra un callback real.
