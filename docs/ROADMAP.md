...
- [ ] `AstroPayAdapter` (`services/payment/src/providers/astropay.adapter.ts`) — construido contra la documentación real de AstroPay (OAuth client_credentials, `POST /v1/payments` Offsite Checkout, verificación de firma RSA vía `GET /v1/certificates`). Auth probada en vivo (200 OK contra `partners-api.astropay.com`). **No wireado en `PaymentRouterService`**: (1) las credenciales recibidas autentican contra producción, no sandbox — a la espera de credenciales de sandbox reales antes de probar creación de pagos o webhooks de punta a punta; (2) falta una decisión de negocio sobre si reemplaza o complementa a Mercado Pago para `AR`. El algoritmo de firma (RSA-SHA256) es una inferencia de "RSA-based signature" — los docs no lo nombran explícito — y debe confirmarse contra un callback real antes de confiar en producción.
