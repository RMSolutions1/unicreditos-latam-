...
- [x] Clientes: ficha 360 de un cliente agregando datos reales de 3 servicios (`identity`, `kyc`, `credit`) en una sola vista — perfil, estado de KYC, historial de solicitudes y créditos. Nuevos endpoints staff-only reutilizando servicios ya existentes sin duplicar lógica: `GET /users/:id` (relajado de SUPER_ADMIN-only a los mismos roles de lectura que el listado — era demasiado restrictivo, encontrado en el camino), `GET /kyc/users/:userId/latest`, `GET /credit-applications/by-user/:userId`, `GET /credits/all?userId=`. Probado en vivo con datos reales de un cliente con 3 solicitudes.
- [x] Contratos: `GET /contracts` nuevo en `credit` (paginado, búsqueda por folio/cliente) — lectura para `TREASURY_MANAGER/COMPLIANCE_MANAGER/SUPER_ADMIN/AUDITOR/CEO/CFO`. `Contract` es 1:1 con `CreditApplication` y append-only por diseño (master prompt §37) — el endpoint es solo lectura a propósito, sin PATCH/DELETE. Probado en vivo: 3 contratos reales con hash de documento y link al crédito desembolsado.
- [x] Notificaciones: nueva tabla `NotificationLog` — `packages/notifications` enviaba emails reales por Resend desde Fase 1 sin dejar rastro persistido. `notify()` ahora loguea cada intento (`SENT`/`FAILED` con el error real de Resend si corresponde) de forma fire-and-forget, sin poder tirar abajo el flujo de negocio que lo llama (mismo criterio que `sendEmail`). `GET /notifications` nuevo en `identity`, lectura para `SUPER_ADMIN/CEO/COMPLIANCE_MANAGER/SUPPORT/AUDITOR`. Probado en vivo: notificación real disparada y logueada, visible en el admin con filtros por tipo/estado.
  - **Hallazgo de auditoría en el camino (2026-09-06)**: al correr `prisma migrate dev` para esta tabla se detectó drift real entre el historial de migraciones y la DB viva — índices de performance en `credit_applications`/`installments`/`payment_intents` y una columna `provider_payload` (jsonb) en `payment_intents`, agregados directo a la DB por fuera de Prisma (probablemente otra sesión/parte del equipo trabajando en paralelo). Se reconcilió `schema.prisma` contra la DB real (sin resetear nada, sin perder datos) y se agregó la tabla nueva vía `prisma migrate deploy` en vez de `migrate dev`, que hubiera pedido resetear la base.
- [ ] Comercios, Compliance, Fraude, Documentos, Reportes, Configuración, Sistema.
